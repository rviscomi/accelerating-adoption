...
function generateFeatureCardHtml(featureId, feature, polyfillData) {
  ...
  const specs = feature.spec || [];
  const specLinks = (Array.isArray(specs) ? specs : [specs])
    .map(spec => `<a href="${escapeHtml(spec)}" target="_blank" rel="noopener noreferrer" class="spec-link">Specification</a>`)
    .join(" · ");
  ...
  return `
        <div class="feature-card">
          ...
          ${specLinks ? `<div class="feature-specs">${specLinks}</div>` : ""}
...
async function generateHtml(polyfillMappings) {
  // Get features with polyfills and sort by baseline date (newest first)
